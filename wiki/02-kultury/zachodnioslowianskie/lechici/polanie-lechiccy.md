# Polanie lechiccy

Polanie lechiccy to dominująca kultura Wielkopolski i serce przyszłego państwa polskiego. Ich dworski etos, tradycje rolnicze i miejskie czynią z nich naturalnych budowniczych królestwa. To kultura Siemowita i dynastii Piastów -- idealna do zjednoczenia ziem lechickich pod jednym berłem.

---

## Karta kultury

| Pole | Wartość |
|------|---------|
| **Nazwa (PL)** | Polanie lechiccy |
| **Klucz w grze** | `lechitic_polans` |
| **Grupa dziedzictwa** | zachodniosłowiańska (`heritage_west_slavic`) |
| **Etos** | Dworski (`ethos_courtly`) |
| **Język** | lechicki (`language_lechitic`) |
| **Lista imion** | `name_list_se_slavic` |
| **Tradycje startowe** | Rolnicy, Strażnicy miast |
| **Zasada sukcesji wojennej** | tylko mężczyźni (`martial_custom_male_only`) |
| **Kolor na mapie** | RGB 0.70 0.45 0.20 |
| **Status w modzie** | nowa |
| **Plik definicji** | `SE/common/culture/cultures/se_slavic_new.txt` |

---

## Etos i tradycje

### Etos: Dworski
Etos dworski kładzie nacisk na dyplomację, sojusze i prestiż dynastyczny. Daje bonusy do zdobywania prestiżu i ułatwia zawieranie sojuszy małżeńskich. To jedyny etos dworski wśród kultur lechickich, co odzwierciedla ambicje polityczne Polan na tle bardziej wspólnotowych lub stoickich sąsiadów.

### Tradycje
- **Rolnicy** (`tradition_agrarian`) -- bonus do rozwoju prowincji i produkcji żywności. Oddaje żyzność gleb wielkopolskich i rolniczy charakter osadnictwa Polan.
- **Strażnicy miast** (`tradition_city_keepers`) -- bonus do rozwoju miast i dochodów z baronii miejskich. Odzwierciedla rolę Gniezna i Poznania jako ośrodków władzy.

---

## Region i prowincje startowe

### Region historyczny
Polanie zajmują Wielkopolskę -- rozległą nizinę wokół Warty i Noteci z centrum w Gnieźnie. Region obejmuje również ziemie łęczyckie i sieradzkie na południowym wschodzie, dając kontrolę nad kluczowym szlakiem z zachodu na wschód.

### Prowincje przypisane w 867
Z pliku `SE/history/provinces/se_lechitic_polans.txt`:

**Księstwo Wielkopolski (d_wielkopolska):**
- prowincja 4900, 4905, 4909, 4912, 3149

**Księstwo Kujaw / ziemie łęczycko-sieradzkie (d_kuyavia):**
- prowincja 4924, 528, 4931

Łącznie **8 prowincji** -- jeden z największych bloków terytorialnych wśród Lechitów.

### Sąsiedztwo kulturowe
- **Na północ:** Goplanie (pokrewni Lechici, ale słabsi) i Kaszubi (morski etos)
- **Na wschód:** Mazowszanie (leśni Lechici, tradycja obronna)
- **Na południe:** Wiślanie (stoiccy, orientacja na zamki) i Ślężanie (nadodrzańscy rolnicy)
- **Na zachód:** Serbowie łużyccy i ludy połabskie (inny język, inna dynamika)

---

## Wiara
- **Wiara startowa (867):** [zachodniosłowiańska](../../03-wiara/przeglad-wiary.md)
- **Najbliższe miejsca święte:** Płock (c_plocka), Legnica (c_legnica), Arkona (c_rugen)

---

## Kontekst historyczny
Polanie to plemię, od którego wywodzi się sama nazwa Polski. Ich gród w Gnieźnie stał się pierwszą stolicą państwa Piastów. W 867 roku znajdują się na etapie konsolidacji władzy pod wodzami takimi jak Siemowit, ale prawdziwy przełom nastąpi dopiero za Mieszka I w X wieku. Mod wyodrębnia ich z waniliowej kultury "polskiej", oddając plemienny charakter epoki.

---

## Porady dla gracza

### Mocne strony
- Największy blok terytorialny wśród Lechitów -- solidna baza do ekspansji
- Etos dworski ułatwia dyplomację i sojusze, których brakuje sąsiadom
- Tradycje rolnicze i miejskie generują stabilny rozwój i dochody
- Siemowit jako postać startowa daje historyczny punkt wyjścia

### Wyzwania
- Otoczenie ze wszystkich stron przez pokrewne, ale niezależne kultury lechickie
- Brak tradycji wojskowej -- ekspansja wymaga inwestycji w armię
- Presja chrześcijaństwa z zachodu w dłuższej perspektywie

---

## Powiązane artykuły
- Grupa dziedzictwa: [Kultury zachodniosłowiańskie](../README.md)
- Wiara: [zachodniosłowiańska](../../03-wiara/przeglad-wiary.md)
- Walka Słowiańszczyzny: [Czym jest](../../04-walka-slowianczyzny/czym-jest-walka.md)
- Postacie startowe: [Siemowit](../../07-scenariusze/postacie/siemowit.md)
