...
>
> **Skąd brać dane:**
> 1. **Definicja decyzji** — `SE/common/decisions/wsl_religious_decisions.txt` (decyzje religijne SE) lub `SSP/common/decisions/ssp_decisions.txt` (decyzje SSP).
...
> 7. **Powiązane scripted_effects** — `SE/common/scripted_effects/wsl_*.txt`, `SSP/common/scripted_effects/`.
>
...
| **Klucz** | `{{wsl_reform_panslavic_faith}}` |
| **Mod źródłowy** | {{SE / SSP}} |
| **Plik** | `{{SE/common/decisions/wsl_religious_decisions.txt}}` |
...
- {{Piety ≥ 2000}}
- {{Prestige ≥ 1500}}
- {{Kontrola miejsc świętych: `wsl_controls_shared_holy_sites_trigger`, `wsl_controls_regional_holy_sites_trigger`}}
- {{Próg pogańskich Słowian w regionie: `wsl_pagan_slavs_threshold_trigger`}}
- {{...}}
...
### Natychmiastowe
- {{Zmiana wiary postaci na `wsl_slavic_unified`}}
- {{Zmiana wiary stolicy + nieformalna konwersja domeny}}
- {{Nadanie modyfikatora dynastycznego `wsl_modifier_unifier_of_faiths`}}
- {{+X piety, +Y prestige (ponad koszt)}}

### Długoterminowe / efekty pasywne
- {{Stała zmiana w mechanice — np. wszystkie hrabstwa z wiarą `wsl_slavic_*` zaczynają konwertować się ku `unified`}}
- {{Modyfikator regionalny: `{{ssp_modifier_xxx}}`}}
- {{Wpływ na Walkę: katalizator `{{nazwa}}` → przesunięcie ku fazie / zakończeniu}}

### Eventy łańcuchowe (jeśli są)
- {{Po N dniach od decyzji uruchamia się event `wsl_event.0001`}}
- {{...}}
